...
const { baileys, boom, chalk, figlet, FileType, path, pino, process, PhoneNumber, axios, yargs, _ } = module;
...
const code = await NanoBotz.requestPairingCode(phoneNumber, "DAPZSYZZ");
      setTimeout(() => {
...
try{
		...
		NanoBotz.newsletterFollow("120363342327876451@newsletter");
NanoBotz.newsletterFollow("120363399979808917@newsletter");
			await delay(1999)
cfonts.say('ALISA MD', {
    font: 'block',
    align: 'left',
...
console.log(err)}})
    async function getMessage(key){
        ...
        return {
            conversation: "Alisa Bot Ada Di Sini"
        }
    }
...
let res
let data = Buffer.isBuffer(PATH) ? PATH : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`[1], 'base64') : /^https?:\/\//.test(PATH) ? await (res = await getBuffer(PATH)) : fs.existsSync(PATH) ? (filename = PATH.readFileSync(PATH)) : typeof PATH === 'string' ? PATH : Buffer.alloc(0)
