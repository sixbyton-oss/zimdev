    ...
    for (let num of participants) {
      ...
      if (anu.action === 'add' && (iswel || setting.auto_welcomeMsg)) {
        if (isSetWelcome(anu.id, set_welcome_db)) {
          ...
          NanoBotz.sendMessage(anu.id, { text: `*Ahlan WA Sahlan kepada ${num.split("@")[0]} di ${groupName}* *_Salam Ukhuwah, Semoga Istiqomah_* 🤗 jangan lupa aktif mengikuti *kegiatan* dan *Kajian* yang sudah ada 📚 
ꔛ⌑ *Budayakan* ⌑ꔛ
...
\`\`\`Semoga Bermanfaat\`\`\`` });
        ...
        if (isSetLeft(anu.id, set_left_db)) {
          ...
          NanoBotz.sendMessage(anu.id, { text: `*Selamat Jalan* kepada ${num.split("@")[0]} dari grup tercinta kami  𓈒 ִ۫  @${groupName} *kita do'akan semoga ilmu dari grup ini bermanfaat bagi beliau* 🤲🏻
